...
const ListBookings = ({ listAll = false }) => {
  ...
  useEffect(() => {
    ...
    const FetchBookingData = async (Populate) => {
      ...
      if (listAll) {
        ...
        const bookings_collection = collection(
          db,
  ...
  const CreateBookingsRows = (filter = null) => {
    ...
    for (let i = 0; i < bookings.length; i++) {
      ...
      if (filter === UPCOMING && (is_request || !booking_obj.IsUpcoming()))
        continue;
      ...
      rows.push(
        <Table.Row key={booking_obj.id}>
          ...
          {filter === null || filter === REQUESTS ? (
            <Table.Cell>{booking_obj.GetStatusString()}</Table.Cell>
          ) : null}
          <Table.Cell>
            <Link
              to={`/dashboard/bookings/view_booking/user_view?id=${booking_obj.id}`}
            >
