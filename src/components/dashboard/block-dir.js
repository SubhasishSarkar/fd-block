...
const BlockDir = () => {
  ...
  if (current_entries.length > 0) {
    ...
    const end_at = Math.min(
      start_from + number_of_entries_per_page,
  ...
  const HandleFilterStringChange = (e) => {
    ...
    let temp_entries = BlockDirectory.filter((entry) => {
      ...
      if (
        entry.name.toUpperCase().indexOf(f.toUpperCase()) > -1 ||
        entry.phoneNumber.indexOf(f) > -1 ||
        pn.indexOf(f) > -1
      ) {
        return true;
      }
  ...
  const total_num_of_pages = Math.ceil(
    current_entries.length / number_of_entries_per_page
  );
  return (
    <div className="mt-3 flex flex-col justify-center flex-auto">
      <div className="flex justify-between">
        ...
        <div className="mb-5">
          <Tooltip
            content="Search by name, phone-number, flat-number"
            placement="top"
          >
            <TextInput
              placeholder="Search"
